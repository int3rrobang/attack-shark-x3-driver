...
| Checked `serde` / state schema 4 | No protocol fixture | `state::model` + `device` tests reject non-canonical `mouse-N`, incoherent `DeviceEndpoint`, unknown `schemaVersion`; `app_settings` schema 1 tested with backup on unreadable | Schema 4 has no migration from schema 3 |
