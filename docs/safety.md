...
- Device identity is logical `mouse-N` (`N >= 1`) stored in `state.json` schema 5 (`nextDeviceNumber` allocation; no migration from schema 4). No serial number or HID path is exposed as a stable identifier; `DeviceEndpoint.serial_number` is trimmed metadata only and HID `UsbPath` is a locator that can change on replug. In persistent identity mode the driver-owned watermark in the DPI tail is the per-unit identifier; see [`logical-mouse-identity-spec.md`](logical-mouse-identity-spec.md).
...
- **Cross-transport linkage comes from physical identity.** Discovery never merges wired/receiver/BLE by model, serial, VID/PID, or connection timing. In persistent identity mode the same watermark observed over wired and receiver resolves to one logical mouse, and a BLE endpoint attaches only through the explicit association ceremony; there is no `link_devices`-style merge API. Do not claim stable-serial or automatic-link behavior.
- **A locator moves only with authentication.** Discovery upserts a known `(transport, locator)` in place; there is no standalone `rebind` command. In persistent identity mode a locator is replaced only by a reappearing endpoint whose current-profile watermark authenticates as the saved physical token; zero, multiple, malformed, unsupported, or foreign candidates are refused — never same-VID/PID guessing. Legacy mode keeps the fuzzy behavior: the old locator returns immediately, otherwise a unique same-VID/PID candidate is accepted after a port change. CLI `use <device>` selects an exact logical ID or a unique display name; `--device <ID>` is required when ambiguity exists.
...
- **Ambiguity refuses to guess.** Resolving a device without an explicit ID when multiple connected logical identities exist returns `AmbiguousDevice` (list of candidates) rather than picking one.
- **Power-cycle verification authenticates by watermark in persistent mode.** Once persistent identity is enabled, the reappearing device is authenticated by its current-profile watermark: exactly one candidate carrying the saved physical token is accepted and its locator stored; zero, multiple, malformed, unsupported, or foreign candidates are refused — the workflow never guesses by model or VID/PID, and an identical-model unit with a different token cannot satisfy the check. Legacy mode (no physical id) keeps the fuzzy model-level behavior: the old locator returns immediately, otherwise a unique same-VID/PID candidate is accepted after a port change, so `PowerCycleVerified` there proves model-level persistence only and cannot distinguish two identical units. Power-cycle verification is USB-only; it is rejected over BLE.
