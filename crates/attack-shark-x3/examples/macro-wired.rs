...
fn build_packets(
    ...
    p2[0] = 0x09;
    p2[1] = if kind == UsbDeviceKind::Receiver {
        0x0C
    } else {
        0x40
    }; // 0x413845 transport-gated
    p2[2] = wire_id;
    p2[3] = 0x02;
    p2[10] = (sum >> 8) as u8;
...
async fn main() -> Result<(), Box<dyn Error>> {
    let (profile, slot, macro_name, kind, dry_run) =
    ...
    if dry_run {
        println!(
            "dry-run profile={} slot={} wireId={:02x} macro={} transport={:?}",
            profile.get(),
            slot,
            slot,
            macro_name,
        ...
        println!(
            "09 P2: {} (checksum {:04x} tail {:02x})",
            hex(&p2),
            ((p2[10] as u16) << 8) | p2[11] as u16,
            p2[1]
        );
        println!(
            "note: 08 slots other than target are zero in dry-run; live run patches via read_buttons for merge"
        );
        return Ok(());
    }
    ...
    let b08_live_bytes = attack_shark_x3::ButtonsReport::encode(&patched_state)
        .as_bytes()
        .to_vec();
    sleep(Duration::from_millis(200)).await;

    // Send slices: wired expects Sleep 200 after each, receiver 1000 per 0x41369B / 0x413853
    let delay = if kind == UsbDeviceKind::Receiver {
    ...
    sleep(delay).await;
    println!(
        "sent P0 09 40 {} 00 ({} bytes) -> transp ack",
        slot,
        p0.len()
    );
    ...
    sleep(delay).await;
    println!(
        "sent P2 09 {:02x} {} 02 checksum {:02x}{:02x}",
        p2[1], slot, p2[10], p2[11]
    );
    handle
        .send_raw_feature_report(b08_live_bytes.clone())
        .await?;
    sleep(delay).await;
    println!(
        "sent 08 bind 12 00 {:02x} for profile {} slot {}",
        slot,
        profile.get(),
        slot
    );
    ...
    println!(
        "done — now press the bound button (slot {}) within 5s to see macro {} fire (A=04/B=05 10ms)",
        slot, macro_name
    );
    Ok(())
}
